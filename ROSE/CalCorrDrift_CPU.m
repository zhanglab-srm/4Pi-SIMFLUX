function [driftx, drifty, dx, dy, xcorr, ycorr] = CalCorrDrift_CPU(x,y,frame,minpts,minframes)
...
%
    ...
    while(start+pnum+step < plen)
        ...
        [dx dy distbuf] = CalculateDrift_CPU(x1, y1, x2, y2);
        driftlen = driftlen +1;
    ...
    if size(driftbuf,1)>1
        ...
        driftx = ones(size(framelist)).*driftbuf(1,1);
        drifty = ones(size(framelist)).*driftbuf(1,2);
    end
