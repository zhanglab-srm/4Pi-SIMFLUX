function [smInfo, procresult] = processHEXFile_series(filename, IsEM, xydrift, forcefit)
...
filenum = 1;
while exist(sprintf('%s_X%d.tif', filename(1:end-4), filenum+1), 'file')
    filenum = filenum +1;
end
if filenum>1
    disp(sprintf('Multi File detected, filenum = %d', filenum));
end
%% load image
...
%% Multi file process
if filenum>1 && not(forcefit == 0 && exist(tmatfile,'file'))
    disp('Multi file Process..');
    clear imgbuf
    parameters = [];
    
    parameters.detection_threshold = detection_threshold;
    parameters.detection_size = detection_size;
    parameters.detection_disk_radius = detection_disk_radius;
    parameters.subimghw = subimghw;
    parameters.frame_offset = s(3);
    
    for filecnt = 2:filenum

        parameters.frame_offset = s(3);
        tfilename = sprintf('%s_X%d.tif', filename(1:end-4), filecnt);
        disp(sprintf('Processing File:%s', tfilename));
        
        result = HEXFileLoadNDetection(tfilename, parameters);
        
        %result: detection_result, moleculecnt, subimginfo, subimglen, subimgbuf, s
        %merge result
        
        detection_result = cat(1, detection_result, result.detection_result);
        moleculecnt = moleculecnt + result.moleculecnt;
        subimginfo = cat(1, subimginfo, result.subimginfo);
        subimglen = subimglen + result.subimglen;
        
        subimgbuf = cat(3,subimgbuf,result.subimgbuf);
        s(3) = s(3) + result.s(3);
...
subplot(3,3,1)
hist(smInfo_md1,50)
title(sprintf('MD1 %0.2f std= %0.2f',mean(smInfo_md1), std(smInfo_md1)))
subplot(3,3,4)
hist(smInfo_md2,50)
title(sprintf('MD2 %0.2f std= %0.2f',mean(smInfo_md2), std(smInfo_md2)))

subplot(3,3,2)
hist(smInfo_int,50)
title(sprintf('Phton Mean %d Median%d',round(mean(smInfo_int)), round(median(smInfo_int))))
...
title(sprintf('Int Ratio, std = %0.2f', std((smInfo_int1-smInfo_int2)./(smInfo_int1+smInfo_int2))))
...
% subfunction for load and detection of HEX file
%parameters: detection_threshold, detection_size, detection_disk_radius,
%subimghw, frame_offset
%
%result: detection_result, moleculecnt, subimginfo, subimglen, subimgbuf, s
function result = HEXFileLoadNDetection(filename, parameters)
    %% init parameters
    detection_threshold = parameters.detection_threshold;
    detection_size = parameters.detection_size;
    detection_disk_radius = parameters.detection_disk_radius;
    subimghw = parameters.subimghw;
    frame_offset = parameters.frame_offset;
    ...
    for m=1:length(detection_result)
        ...
        for n=1:size(tbuf,1)
            ...
            subimginfo(subimglen, 1) = tx;
            subimginfo(subimglen, 2) = ty;
            subimginfo(subimglen, 3) = m+frame_offset;
        end
    end
    moleculecnt = subimglen;
    ...
    result.subimglen = subimglen;
    result.subimgbuf = subimgbuf;
    result.subimginfo = subimginfo;
    result.moleculecnt = moleculecnt;
    result.s = s;
end