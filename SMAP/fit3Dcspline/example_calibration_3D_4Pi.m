...
folder='E:\Two-color\20231018\test\test';
files=dir([folder filesep '*.tif']);
N=length(files);
for i=1:N
    p.filelist{i}=[folder filesep files(i).name];
end
p.outputfile = [folder filesep 'bead_astig_3dcal_20231018_560nm.mat' ]; %output file name
...
p.dz =20; %distance between frames in nm
...
p.ROIxy = 25; %size of the PSF model in x,y (pixels)

p.smoothz =1; %smoothing parameter in Z
p.gaussrange =[-600 600]; %z range (nm) in which to calibrate the parameters for Gaussian z-fit.
p.gaussroi = 31; %size of the ROI in pixels for the Gaussian calibration
