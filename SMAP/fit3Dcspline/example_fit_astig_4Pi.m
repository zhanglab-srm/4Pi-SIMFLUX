...
cal=load('E:\Two-color\20231018\test\test\bead_astig_3dcal_20231018_560nm.mat'); %load bead calibration
...
%     file=[pwd filesep 'example_data' filesep 'single_bead.tif']; %experimental astgmatic bead stack.
    file='E:\Two-color\20231018\test\test\1\Cell03_561_060_000-1.tif';
    imstackadu=readfile_tif(file); % Stack of ROIs in photons.
    p.offset=100; p.conversion=4.25;
    imstack=(single(imstackadu)-p.offset)/p.conversion;% if necessary, convert ADU into photons.
    ground_truth.z=((1:size(imstack,3))'-size(imstack,3)/2+1)*20; %dz=10 nm; convert frame to nm   
end
...
%plot fitted z vs ground-truth z
    ...
    subplot(1,2,1);
    hold on
    plot(cspline.x-cspline.x(1))
    plot(cspline.y-cspline.y(1))
    subplot(1,2,2);
    hold on
    plot(gausssxsy.x-gausssxsy.x(1))
    plot(gausssxsy.y-gausssxsy.y(1))
