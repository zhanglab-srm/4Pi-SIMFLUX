function [drift,driftinfo,fieldc]=driftcorrection3D_so(x,y,z,frame,p)
pard.correctxy=true; %correct in x, y
pard.drift_timepoints=70; %number of time points (blocks) in x,y,
pard.drift_pixrec=10; %size of pixels for reconstruction (in units of x,y, e.g. nm)
pard.drift_window=7; %Size of window in which maximum of cross-correlation is found (in reconstructed pixels).
pard.drift_maxdrift=500; %maximum drift (in units of x, eg.g nm)
pard.drift_maxpixels=4096; %maximum size of reconstructed image (to avoid memory overflow).

pard.correctz=true; % correct in z;
pard.drift_timepointsz=120; %time points in z
pard.drift_pixrecz=10; %pixelsize for reconstruction in z (nm)
pard.drift_windowz=9; %size of window used for maximum finding
pard.zrange=[-200 200]; %range in z used for drift correction;
