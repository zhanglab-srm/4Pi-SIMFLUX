function [z_ang,ang_contrast]=W4PiSMS_RM_zangv4(anglefile,q1,q2,q3,q4,locmaxc_f,subsz,xf,yf,fmfile,tlz_f,bg_f,channel)
ldtmp=load(fmfile);
ldtmp.R(:,:,1)=eye(3);
...
varim=single(ones(sz));
gainim=single(zeros(sz))+4.35;
...
if channel == '642'
    int_est(:,3)=int_est(:,3)*0.80;
    int_est(:,4)=int_est(:,4)*0.85;
else  % 561 nm
    int_est(:,3)=int_est(:,3)*0.93;
    int_est(:,4)=int_est(:,4)*0.91;
end
...
figure;
scatter(rms(~id),rmp(~id),1,ang_contrast(~id));
xlabel('rms');
ylabel('rmp');
pause(eps)