...
iter_times = 100;
Npixels = 15;
Nphotons_total = logspace(2,5,Nmol);
bg_total = 12 * 2;
...
ground_truth.N = Nphotons;
ground_truth.bg =repmat(bg, [Nmol 1]);
ground_truth.znm = 0*rand([Nmol 1]);
...
phi_save = zeros(length(zrange),1);
...
plot(Nphotons_total, RMSEX)
loglog(Nphotons_total, RMSEX)
hold on
plot(Nphotons_total, RMSEY)
loglog(Nphotons_total, RMSEY)
hold on
plot(Nphotons_total, RMSEZ)
loglog(Nphotons_total, RMSEZ)

data_save.photons = Nphotons_total;
data_save.STD_x = RMSEX;
data_save.STD_y = RMSEY;
data_save.STD_z = RMSEZ;
save('Fitting_std_4pi_SIMFLUX_compare.mat','data_save');