...
%%
function [out] = simSplinePSF(Npixels,PSF,I,bg,cor)
if (nargin <5)
...
Nfits = size(cor,1);
spline_xsize = size(PSF,1);
spline_ysize = size(PSF,2);
spline_zsize = size(PSF,3);
...
%                  for p = 1:1:length(phi0)
                     temp = fAt3Dj_v2(ii+xstart+off,jj+ystart+off,zstart,spline_xsize,spline_ysize,spline_zsize,delta_f,PSF);
                     model = temp * I(kk) + bg(kk);
